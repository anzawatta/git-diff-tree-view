...
- **リネーム検出（`-M`）の完全対応**: `git diff --numstat -M` が出力する `old => new` 形式のパスは現時点で未解決。リネームを含む diff を扱う場合は `-M` オプションを付けないことを推奨する

## Display Order: 表示順の方針

ツリーの表示順は `git diff --numstat` の出力順（= gitが生成するパス辞書順）を保持する。ツール側でソートを行わないことで、git の順序を信頼源（source of truth）とする。
